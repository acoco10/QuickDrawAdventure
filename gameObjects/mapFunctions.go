...
func DrawMapBelowPlayer(tileMapJson TilemapJSON, tilesets []Tileset, cam camera.Camera, screen *ebiten.Image) {
...
func DrawMapAbovePlayer(tileMapJSON TilemapJSON, tilesets []Tileset, cam camera.Camera, screen *ebiten.Image, player Character) {
	opts := ebiten.DrawImageOptions{}
	for _, layer := range tileMapJSON.Layers {
		if layer.Type == "objectgroup" {
			continue
